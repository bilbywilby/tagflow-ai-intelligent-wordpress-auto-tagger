...
import { fetchEvents, fetchBriefing, syncHubPro, fetchHubStats, fetchGeofences } from '@/lib/api';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Input } from '@/components/ui/input';
import { Sparkles, MapPin, ExternalLink, RefreshCw, Activity, Clock, Search, Navigation } from 'lucide-react';
...
export function Hub() {
  ...
  const loadHubData = React.useCallback(async () => {
    try {
      const [eventsData, briefingData, statsData, geoData] = await Promise.all([
        fetchEvents(hubSelectedLocation || undefined, undefined, hubSearchQuery, hubSelectedNeighborhood || undefined),
        fetchBriefing(),
        fetchHubStats(),
        fetchGeofences()
      ]);
  ...
  }, [hubSelectedLocation, hubSearchQuery, hubSelectedNeighborhood, setEvents, setBriefing, setHubStats, setGeofences]);
  React.useEffect(() => {
    loadHubData();
  }, [loadHubData]);
  ...
  const filteredNeighborhoods = React.useMemo(() => {
    if (!hubSelectedLocation || hubSelectedLocation === 'Greater LV' || hubSelectedLocation === 'Other') return [];
    return geofences.filter(f => f.canonicalPlace === hubSelectedLocation);
  }, [geofences, hubSelectedLocation]);
  return (
        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <div className="flex items-center gap-2 mb-2">
               ...
               <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Neighborhood Intel Active</span>
            </div>
            <h1 className="text-4xl font-display font-bold tracking-tight">
              Lehigh Valley <span className="text-indigo-600">Hub</span>
            </h1>
            <p className="text-muted-foreground text-lg">Hyper-local regional intelligence engine.</p>
          </div>
          <Button
          ...
          <AnimatePresence>
            {filteredNeighborhoods.length > 0 && (
              <motion.div
                ...
                className="flex items-center gap-3 bg-indigo-50/50 dark:bg-indigo-950/20 p-4 rounded-2xl overflow-hidden"
              >
                <Navigation className="size-4 text-indigo-500 shrink-0" />
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
                  <Button
                    variant={hubSelectedNeighborhood === null ? 'outline' : 'ghost'}
                    size="sm"
                    className="h-8 text-[10px] font-bold uppercase tracking-wider"
                    onClick={() => setHubSelectedNeighborhood(null)}
                  >
                    All Neighborhoods
                  </Button>
                  {filteredNeighborhoods.map(fence => (
                    <Button
                      key={fence.id}
                      variant={hubSelectedNeighborhood === fence.name ? 'default' : 'ghost'}
                      size="sm"
                      onClick={() => setHubSelectedNeighborhood(fence.name)}
          ...
          <TabsContent value="all" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <AnimatePresence mode="popLayout">
              {events.map((event) => (
                ...
                >
                  <Card className="h-full flex flex-col group hover:shadow-2xl transition-all duration-500 border-none shadow-soft bg-white dark:bg-slate-900/50">
                    <CardHeader className="pb-3">
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex gap-2">
                          ...
                          {event.neighborhood && (
                            <Badge variant="outline" className="text-[10px] font-bold border-indigo-200 text-indigo-500">
                              {event.neighborhood}
                            </Badge>
                          )}
                        </div>
                    ...
                    <CardContent className="flex-1">
                      ...
                      <div className="space-y-2 pt-4 border-t border-dashed">
                        ...
                        <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest pl-5">
                          {event.location} {event.zipCode ? `• ${event.zipCode}` : ''}
                        </div>
                      </div>
                    </CardContent>
                    <div className="p-6 pt-0">
                      <Button variant="outline" className="w-full h-10 gap-2 text-xs font-bold rounded-xl" asChild>
